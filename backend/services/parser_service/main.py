"""Parser service main application."""
...
app = FastAPI(
    ...
    description="PDF parsing microservice for AI Research Paper Assistant",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def health_check():
    return {"status": "healthy", "service": "parser_service"}
...
if __name__ == "__main__":
    ...
    from backend.services.parser_service.config import SERVICE_PORT

    uvicorn.run(
        "backend.services.parser_service.main:app",
        host="0.0.0.0",
        port=SERVICE_PORT,
        reload=True,
    )
