"""Chunk service main application."""
...
async def lifespan(app: FastAPI):
    logger.info("Chunk service starting up...")
    yield
    logger.info("Chunk service shutting down...")


app = FastAPI(
    title="Chunk Service",
    description="Text chunking microservice for AI Research Paper Assistant",
    version="1.0.0",
    lifespan=lifespan,
)
...
if __name__ == "__main__":
    ...
    from backend.services.chunk_service.config import SERVICE_PORT

    uvicorn.run(
        "backend.services.chunk_service.main:app",
        host="0.0.0.0",
        port=SERVICE_PORT,
        reload=True,
    )
