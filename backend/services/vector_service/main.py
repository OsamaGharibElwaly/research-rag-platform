...
from backend.services.vector_service.routers import router as vector_router
...
async def lifespan(app: FastAPI):
    ...
    yield
    logger.info("Vector service shutting down...")


app = FastAPI(
    title="Vector Service",
    description="Vector indexing and search microservice for AI Research Paper Assistant",
    version="1.0.0",
    lifespan=lifespan,
)
...
if __name__ == "__main__":
    ...
    from backend.services.vector_service.config import SERVICE_PORT

    uvicorn.run(
        "backend.services.vector_service.main:app",
        host="0.0.0.0",
        port=SERVICE_PORT,
        reload=True,
    )
